...
#ifdef RNWebGL_DEBUG
#ifdef __ANDROID__
#define RNWebGLSysLog(fmt, ...) __android_log_print(ANDROID_LOG_DEBUG, "RNWebGL", fmt, ##__VA_ARGS__)
#endif
#ifdef __APPLE__
#define RNWebGLSysLog(fmt, ...) EXiOSLog("RNWebGL: " fmt, ##__VA_ARGS__)
#endif
#else
#define RNWebGLSysLog(...)
#endif
...
// Class of the C++ object representing an RNWebGL rendering context.
...
private:
  ...
  // [JS thread] Enqueue a function and return an RNWebGL object that will get mapped
  ...
  inline JSValueRef addFutureToNextBatch(JSContextRef jsCtx, F &&f) noexcept {
    auto id = createObject();
    addToNextBatch([=] {
      //assert(objects.find(id) == objects.end());
      mapObject(id, f());
    });
    return JSValueMakeNumber(jsCtx, id);
  }
...
private:
  std::unordered_map<RNWebGLTextureId, GLuint> objects;
  static std::atomic_uint nextObjectId;

  inline GLuint lookupObject(RNWebGLTextureId id) noexcept {
    auto iter = objects.find(id);
    return iter == objects.end() ? 0 : iter->second;
  }

public:
  inline RNWebGLTextureId createObject(void) noexcept {
    return nextObjectId++;
  }

  inline void destroyObject(RNWebGLTextureId id) noexcept {
    objects.erase(id);
  }

  inline void mapObject(RNWebGLTextureId id, GLuint glObj) noexcept {
    objects[id] = glObj;
  }
...
#define GL_DEPTH_STENCIL_ATTACHMENT 0x821A
  ...
  static inline void jsThrow(JSContextRef jsCtx, const char *msg, JSValueRef *jsException) {
    *jsException = JSValueToObject(jsCtx, JSValueMakeStringFromUTF8CString(jsCtx, msg), nullptr);
  }

  static inline std::shared_ptr<char> jsValueToSharedStr(JSContextRef jsCtx, JSValueRef jsVal) noexcept {
    return std::shared_ptr<char>(JSValueToUTF8CStringMalloc(jsCtx, jsVal, nullptr), free);
  }
...
#define _WRAP_METHOD(name, minArgc)                                     \
  ...
  {                                                                     \
    auto exglCtx = RNWebGLContextGet((RNWebGLContextId) (intptr_t)           \
...
#define _WRAP_METHOD_UNIMPL(name)                                       \
  _WRAP_METHOD(name, 0) {                                               \
    throw std::runtime_error("RNWebGL: " #name "() isn't implemented yet!"); \
    return nullptr;                                                     \
  }

  // Wrapper that takes only scalar arguments and returns nothing
#define _WRAP_METHOD_SIMPLE(name, glFunc, ...)                          \
  _WRAP_METHOD(name, JS_ARGC(__VA_ARGS__)) {                          \
    addToNextBatch(std::bind(glFunc, JS_MAP_EXT(0, _JS_COMMA, _WRAP_METHOD_SIMPLE_UNPACK, __VA_ARGS__))); \
    return nullptr;                                                     \
  }
#define _WRAP_METHOD_SIMPLE_UNPACK(i, _) JSValueToNumberFast(jsCtx, jsArgv[i])
  ...
  _WRAP_METHOD(getContextAttributes, 0) {
    ...
    JSObjectSetValueWithUTF8CStringName(jsCtx, jsResult, "alpha",
                                          JSValueMakeBoolean(jsCtx, true));
    JSObjectSetValueWithUTF8CStringName(jsCtx, jsResult, "depth",
                                          JSValueMakeBoolean(jsCtx, true));
    JSObjectSetValueWithUTF8CStringName(jsCtx, jsResult, "stencil",
                                          JSValueMakeBoolean(jsCtx, false));
    JSObjectSetValueWithUTF8CStringName(jsCtx, jsResult, "antialias",
                                          JSValueMakeBoolean(jsCtx, false));
    JSObjectSetValueWithUTF8CStringName(jsCtx, jsResult, "premultipliedAlpha",
                                          JSValueMakeBoolean(jsCtx, false));
    return jsResult;
  }
  ...
  _WRAP_METHOD(getParameter, 1) {
    ...
    switch (pname) {
      ...
      case GL_VERSION: {
        ...
        return JSValueMakeStringFromUTF8CString(jsCtx, (const char *) glStr);
      }
...
#define _GET_PARAMETER_UNIMPL(param)                                         \
      case GL_##param:                                                       \
        throw std::runtime_error("RNWebGL: getParameter() doesn't support gl."  \
                                 #param " yet!");
...
#undef _GET_PARAMETER_UNIMPL
  ...
  _WRAP_METHOD(pixelStorei, 2) {
    JS_UNPACK_ARGV(GLenum pname, GLint param);
    switch (pname) {
      case GL_UNPACK_FLIP_Y_WEBGL:
        unpackFLipY = param;
        break;
  ...
  _WRAP_METHOD(bindBuffer, 2) {
    JS_UNPACK_ARGV(GLenum target, RNWebGLTextureId fBuffer);
    addToNextBatch([=] { glBindBuffer(target, lookupObject(fBuffer)); });
    return nullptr;
  }

  _WRAP_METHOD(bufferData, 3) {
    GLenum target = JSValueToNumberFast(jsCtx, jsArgv[0]);
    auto jsSecond = jsArgv[1];
    GLenum usage = JSValueToNumberFast(jsCtx, jsArgv[2]);

    if (JSValueIsNumber(jsCtx, jsSecond)) {
      GLsizeiptr length = JSValueToNumberFast(jsCtx, jsSecond);
  ...
  _WRAP_METHOD(bufferSubData, 3) {
    if (!JSValueIsNull(jsCtx, jsArgv[2])) {
      JS_UNPACK_ARGV(GLenum target, GLintptr offset);
      size_t length;
  ...
  _WRAP_METHOD(deleteBuffer, 1) {
    JS_UNPACK_ARGV(RNWebGLTextureId fBuffer);
    addToNextBatch([=] {
  ...
  _WRAP_METHOD(getBufferParameter, 2) {
    JS_UNPACK_ARGV(GLenum target, GLenum pname);
    GLint glResult;
...
#define _WRAP_METHOD_IS_OBJECT(type)              \
  _WRAP_METHOD(is ## type, 1) {                   \
    JS_UNPACK_ARGV(RNWebGLTextureId f);             \
    GLboolean glResult;                           \
  ...
  _WRAP_METHOD(bindFramebuffer, 2) {
    ...
    if (JSValueIsNull(jsCtx, jsArgv[1])) {
      ...
      RNWebGLTextureId fFramebuffer = JSValueToNumberFast(jsCtx, jsArgv[1]);
  ...
  _WRAP_METHOD(deleteFramebuffer, 1) {
    JS_UNPACK_ARGV(RNWebGLTextureId fFramebuffer);
    addToNextBatch([=] {
  ...
  _WRAP_METHOD(readPixels, 7) {
    JS_UNPACK_ARGV(GLint x, GLint y, GLsizei width, GLsizei height, GLenum format, GLenum type);
    if (usingTypedArrayHack) {
  ...
  _WRAP_METHOD(bindTexture, 2) {
    ...
    if (JSValueIsNull(jsCtx, jsArgv[1])) {
      ...
      RNWebGLTextureId fTexture = JSValueToNumberFast(jsCtx, jsArgv[1]);
  ...
  _WRAP_METHOD(deleteTexture, 1) {
    JS_UNPACK_ARGV(RNWebGLTextureId fTexture);
    addToNextBatch([=] {
  ...
  _WRAP_METHOD(attachShader, 2) {
    JS_UNPACK_ARGV(RNWebGLTextureId fProgram, RNWebGLTextureId fShader);
    addToNextBatch([=] { glAttachShader(lookupObject(fProgram), lookupObject(fShader)); });
    return nullptr;
  }

  _WRAP_METHOD(bindAttribLocation, 3) {
    JS_UNPACK_ARGV(RNWebGLTextureId fProgram, GLuint index);
  ...
  _WRAP_METHOD(compileShader, 1) {
    JS_UNPACK_ARGV(RNWebGLTextureId fShader);
  ...
  _WRAP_METHOD(deleteProgram, 1) {
    JS_UNPACK_ARGV(RNWebGLTextureId fProgram);
  ...
  _WRAP_METHOD(getAttachedShaders, 1) {
    ...
    for (auto i = 0; i < count; ++i) {
      ...
      if (id == 0) {
        throw new std::runtime_error("RNWebGL: Internal error: couldn't find RNWebGLTextureId "
                                     "associated with shader in getAttachedShaders()!");
      }
      jsResults[i] = JSValueMakeNumber(jsCtx, id);
    }
    return JSObjectMakeArray(jsCtx, count, jsResults, nullptr);
  }

  _WRAP_METHOD(getProgramParameter, 2) {
    JS_UNPACK_ARGV(RNWebGLTextureId fProgram, GLenum pname);
    GLint glResult;
  ...
  _WRAP_METHOD(getShaderParameter, 2) {
    JS_UNPACK_ARGV(RNWebGLTextureId fShader, GLenum pname);
    GLint glResult;
  ...
  _WRAP_METHOD(getShaderPrecisionFormat, 2) {
    JS_UNPACK_ARGV(GLenum shaderType, GLenum precisionType);
    ...
    JSObjectSetValueWithUTF8CStringName(jsCtx, jsResult, "rangeMin",
                                          JSValueMakeNumber(jsCtx, range[0]));
    JSObjectSetValueWithUTF8CStringName(jsCtx, jsResult, "rangeMax",
                                          JSValueMakeNumber(jsCtx, range[1]));
    JSObjectSetValueWithUTF8CStringName(jsCtx, jsResult, "precision",
  ...
  inline JSValueRef getShaderOrProgramStr(JSContextRef jsCtx, const JSValueRef jsArgv[],
    ...
    JS_UNPACK_ARGV(RNWebGLTextureId fObj);
    GLint length;
    std::string str;
    addBlockingToNextBatch([&] {
    ...
    return JSValueMakeStringFromUTF8CString(jsCtx, str.c_str());
  }
  ...
  inline JSValueRef getActiveInfo(JSContextRef jsCtx, const JSValueRef jsArgv[],
    ...
    JSObjectRef jsResult = JSObjectMake(jsCtx, nullptr, nullptr);
    JSObjectSetValueWithUTF8CStringName(jsCtx, jsResult, "name",
                                          JSValueMakeStringFromUTF8CString(jsCtx, name.c_str()));
    JSObjectSetValueWithUTF8CStringName(jsCtx, jsResult, "size", JSValueMakeNumber(jsCtx, size));
    JSObjectSetValueWithUTF8CStringName(jsCtx, jsResult, "type", JSValueMakeNumber(jsCtx, type));
    return jsResult;
  }
...
#define _WRAP_METHOD_UNIFORM_V(suffix, dim, Type)                     \
  _WRAP_METHOD(uniform##suffix, 2) {                                  \
    GLuint uniform = JSValueToNumberFast(jsCtx, jsArgv[0]);         \
    size_t bytes;                                                     \
...
#define _WRAP_METHOD_VERTEX_ATTRIB_V(suffix, dim)                       \
  _WRAP_METHOD(vertexAttrib##suffix, 2) {                               \
    GLuint index = JSValueToNumberFast(jsCtx, jsArgv[0]);             \
...
#undef _WRAP_METHOD_VERTEX_ATTRIB_V
  ...
  _WRAP_METHOD(vertexAttribPointer, 6) {
    JS_UNPACK_ARGV(GLuint index, GLuint itemSize, GLenum type,
  ...
  _WRAP_METHOD(drawElements, 4) {
    JS_UNPACK_ARGV(GLenum mode, GLsizei count, GLenum type, GLint offset);
  ...
  // "private" function that will be hooked in getExtension("RNWebGL")
...
#define _INSTALL_METHOD(name)                                           \
    JSObjectSetFunctionWithUTF8CStringName(jsCtx, jsGl, #name,        \
                                             &RNWebGLContext::exglNativeStatic_##name)
...
#define _INSTALL_CONSTANT(name)                                         \
    JSObjectSetValueWithUTF8CStringName(jsCtx, jsGl, #name,           \
...
std::atomic_uint RNWebGLContext::nextObjectId { 1 };
...
static RNWebGLContext *RNWebGLContextGet(RNWebGLContextId ctxId) {
  ...
  auto iter = RNWebGLContextMap.find(ctxId);
...
RNWebGLContextId RNWebGLContextCreate(JSGlobalContextRef jsCtx) {
  // Out of ids?
  if (RNWebGLContextNextId >= std::numeric_limits<RNWebGLContextId>::max()) {
    RNWebGLSysLog("Ran out of RNWebGLContext ids!");
    return 0;
  }
  ...
  auto jsRNWebGLContextMap = (JSObjectRef) JSObjectGetPropertyNamed(jsCtx, jsGlobal, "__RNWebGLContexts");
  if (!JSValueToBoolean(jsCtx, jsRNWebGLContextMap)) {
    jsRNWebGLContextMap = JSObjectMake(jsCtx, nullptr, nullptr);
    JSObjectSetValueWithUTF8CStringName(jsCtx, jsGlobal, "__RNWebGLContexts", jsRNWebGLContextMap);
  }
  std::stringstream ss;
  ss << ctxId;
  auto ctxIdStr = ss.str();
  JSObjectSetValueWithUTF8CStringName(jsCtx, jsRNWebGLContextMap,
                                        ctxIdStr.c_str(), gl->getJSObject());
...
void RNWebGLContextDestroyObject(RNWebGLContextId ctxId, RNWebGLTextureId objId) {
  auto exglCtx = RNWebGLContextGet(ctxId);
  if (exglCtx) {
    exglCtx->destroyObject(objId);
  }
}

void RNWebGLContextMapObject(RNWebGLContextId ctxId, RNWebGLTextureId objId, GLuint glObj) {
  auto exglCtx = RNWebGLContextGet(ctxId);
  if (exglCtx) {
